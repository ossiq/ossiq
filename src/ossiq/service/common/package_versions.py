...
def aggregated_package_versions(
    ...
    """
    package_info = sources.packages_registry.package_info(package_info.name)

    source_code_provider_type = detect_source_code_provider(package_info.repo_url)
    source_code_provider = sources.get_source_code_provider(source_code_provider_type)
    # Leveraging abstractions to the full extend
    package_versions = list(sources.packages_registry.package_versions(package_info.name))
    ...
    if package_info.latest_version:
        versions_delta = list(
            filter_versions_between(
                ...
                comparator=sources.packages_registry.compare_versions,
            )
        )
    else:
    ...
    repository_versions = list(
        source_code_provider.repository_versions(
            repository_info, packages_delta, comparator=sources.packages_registry.compare_versions
        )
    )
...
def package_changes(sources: AbstractProjectSources, package_name: str, installed_version: str) -> Iterable[Version]:
    """
    Aggregate changes between two versions of a package regardless of the registry.
    """

    package_info = sources.packages_registry.package_info(package_name)
    repository_provider = sources.get_source_code_provider(detect_source_code_provider(package_info.repo_url))
    ...
    package_versions, repository_versions = aggregated_package_versions(
        sources, repository_info, package_info, installed_version
    )
    ...
    for package_version in package_versions:
        ...
        yield Version(
            package_registry=sources.packages_registry.package_registry,
