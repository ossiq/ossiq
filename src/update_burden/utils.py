...
from .diffs import diff_exports, render_inline_diff
from .ast_parser import parse_files_with_tree_sitter, JS_LANGUAGE
...
def print_report(diffs):
    console.print(Panel(Text("API Diff Report", style="bold cyan"), expand=False))
    ...
    for category, (removed, added, changed) in diffs.items():
        ...
        if changed:
            ...
            for f, diff in changed.items():
                ...
                sig_diff = render_inline_diff(diff["old"]["signature"], diff["new"]["signature"])
                ...
                if diff["old"]["doc"] or diff["new"]["doc"]:
                    doc_diff = render_inline_diff(diff["old"]["doc"], diff["new"]["doc"])
                    console.print("[bold]Doc Diff:[/bold]")
                    console.print(doc_diff)


def run_analysis(repo_v1, repo_v2):
    exports_v1 = parse_files_with_tree_sitter(repo_v1, JS_LANGUAGE)
