...
def aggregated_package_versions(
        ...
        latest_version: str) -> Tuple[List[PackageVersion], List[RepositoryVersion]]:
    """
    ...
    if not package_versions:
        raise NoPackageVersionsFound(
            f"Cannot load package versions for {package_name}")

    # NOTE: we don't need to pull all the versions, just the difference between
    # what we have and what is the latest available.
    versions_delta = list(filter_versions_between(
    ...
    packages_delta = [
        p for p in package_versions
        if p.version in versions_delta]

    repository_versions = list(repository_provider.repository_versions(
        repository_info,
        packages_delta
    ))
    ...
    if repository_versions is None:
        raise NoPackageVersionsFound(
            f"Cannot load repository versions for {package_name}")
...
def package_changes(
        uow: AbstractProjectUnitOfWork,
        package_name: str,
        installed_version: str) -> Iterable[Version]:
    """
    ...
    repository_info = repository_provider.repository_info(
