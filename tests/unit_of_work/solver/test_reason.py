...
class TestBuildReasonCombined:
        ...
        problem = _sp(
            [_pc("pkg", version_constraint=">=1.0,<3.0")],
            {
                "pkg": [
                    ...
                    _cv("2.8.1", age_days=45),  # selected
                ]
            },
        )
    ...
    def test_age_preference_only_produces_no_rejection_entry(self) -> None:
        """Newer eligible candidate loses only on L3 age score — no soft_rejection emitted."""
        problem = _sp(
            [_pc("pkg")],
            {
                "pkg": [
                    _cv("2.0.0", age_days=5),  # very fresh but penalize_fresh_days=0 → L3 only
                    _cv("1.0.0", age_days=200),  # selected (higher age_weight)
                ]
            },
        )
        reason = build_reason("pkg", "1.0.0", problem, penalize_fresh_days=0)
