  ...
  function buildNode(key: string, versionDefinedOverride?: string): D3NodeData {
    ...
    if (vnode.directName !== null) {
      ...
      return {
        ...
        recommended_version: de?.recommended_version ?? null,
        next_action: de?.next_action ?? null,
        requires_constraint_widening: de?.requires_constraint_widening ?? false,
        recommended_from_rung: de?.recommended_from_rung ?? null,
        latest_in_range: de?.latest_in_range ?? null,
        latest_in_major: de?.latest_in_major ?? null,
        latest_compatible_major: de?.latest_compatible_major ?? null,
        latest_preserving_module_system: de?.latest_preserving_module_system ?? null,
        module_system_note: de?.module_system_note ?? null,
        version_constraint_declared: de?.version_constraint_declared ?? null,
        epss: de?.epss ?? null,
        ...
        dependency_health_action: de?.dependency_health_action ?? null,
    ...
    return {
      ...
      // Transitive metrics carry no recommendation or rung of their own — the scan solves them
      // through their parents — so only the ladder rungs and the next action are meaningful here.
      next_action: re?.next_action ?? null,
      latest_in_range: re?.latest_in_range ?? null,
      latest_in_major: re?.latest_in_major ?? null,
      latest_compatible_major: re?.latest_compatible_major ?? null,
      latest_preserving_module_system: re?.latest_preserving_module_system ?? null,
      module_system_note: re?.module_system_note ?? null,
      epss: re?.epss ?? null,
      ...
      dependency_health_action: re?.dependency_health_action ?? null,
