...
export function buildPackageRegistry(report: OSSIQExportSchemaV15): PackageRegistry {
  ...
  for (const pkg of report.transitive_packages) {
    byId.set(pkg.id, {
      ...
      next_action: pkg.next_action ?? null,
      ...
      maintenance_state: pkg.maintenance_state ?? null,
  ...
  for (const pkg of report.production_packages) {
    directEntries.set(pkg.package_name, {
      ...
      is_package_unpublished: pkg.is_package_unpublished ?? false,
      next_action: pkg.next_action ?? null,
      requires_constraint_widening: pkg.requires_constraint_widening ?? false,
      recommended_version: pkg.recommended_version ?? null,
      recommended_from_rung: pkg.recommended_from_rung ?? null,
      latest_in_range: pkg.latest_in_range ?? null,
      latest_in_major: pkg.latest_in_major ?? null,
      latest_compatible_major: pkg.latest_compatible_major ?? null,
